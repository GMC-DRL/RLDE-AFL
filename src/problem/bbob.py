...
class F4(BBOB_Basic_Problem):
    ...
    def func(self, x):
        ...
        even[even > 0.] *= 10.
        z *= self.scales
        return 10 * (self.dim - np.sum(np.cos(2 * np.pi * z), axis = -1)) + np.sum(z ** 2, axis = -1) + 100 * pen_func(
            x, self.ub) + self.bias
...
class _Rosenbrock(BBOB_Basic_Problem):
    ...
    def func(self, x):
        ...
        return np.sum(100 * (z[:, :-1] ** 2 - z[:, 1:]) ** 2 + (z[:, :-1] - 1) ** 2,
                      axis = -1) + self.bias + self.boundaryHandling(x)
...
class _Dif_powers(BBOB_Basic_Problem):
    ...
    def func(self, x):
        ...
        return np.power(np.sum(np.power(np.fabs(z), 2 + 4 * i / max(1, self.dim - 1)), -1),
...
class F16(BBOB_Basic_Problem):
    ...
    def func(self, x):
        ...
        return 10 * np.power(
            np.mean(np.sum(self.aK * np.cos(np.matmul(2 * np.pi * (z[:, :, None] + 0.5), self.bK[None, :])), axis = -1),
                    axis = -1) - self.f0, 3) + \
...
class _Scaffer(BBOB_Basic_Problem):
    ...
    def func(self, x):
        ...
        return np.power(
            1 / (self.dim - 1) * np.sum(np.sqrt(s) * (np.power(np.sin(50 * np.power(s, 0.2)), 2) + 1), axis = -1), 2) + \
               self.boundaryHandling(x) + self.bias
...
class _Composite_Grie_rosen(BBOB_Basic_Problem):
    ...
    def func(self, x):
        ...
        return self.factor + self.factor * np.sum(s / 4000. - np.cos(s), axis = -1) / (
...
class F20(BBOB_Basic_Problem):
    ...
    def func(self, x):
        ...
        b = 4.189828872724339
        return b - 0.01 * np.mean(z * np.sin(np.sqrt(np.abs(z))), axis = -1) + 100 * pen_func(z / 100,
                                                                                              self.ub) + self.bias
...
class F24(BBOB_Basic_Problem):
    ...
    def func(self, x):
        ...
        return np.minimum(np.sum((x_hat - self.mu0) ** 2., axis = -1),
                          self.dim + s * np.sum((x_hat - mu1) ** 2., axis = -1)) + \
